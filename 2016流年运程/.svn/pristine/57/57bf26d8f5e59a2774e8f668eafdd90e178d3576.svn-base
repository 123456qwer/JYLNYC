...
-(NSDictionary *)jar:(id)d{
    ...
    }else if([d isEqualToString:@"平"]){ yjdict = @{@"宜":@"祭祀 修墳 涂泥 餘事勿取", @"忌":@"移徙 入宅 嫁娶 开市 安葬"};
        
    }else if([d isEqualToString:@"定"]){ yjdict = @{@"宜":@"交易 立券 会友 签約 納畜", @"忌":@"种植 置业 卖田 掘井 造船"};
        
    }else if([d isEqualToString:@"执"]){ yjdict = @{@"宜":@"祈福 祭祀 求子 结婚 立约", @"忌":@"开市 交易 搬家 远行"};
        
    }else  if([d isEqualToString:@"破"]){yjdict = @{@"宜":@"求医 赴考 祭祀 餘事勿取", @"忌":@"动土 出行 移徙 开市 修造"};
        
    }else  if([d isEqualToString:@"危"]){ yjdict = @{@"宜":@"经营 交易 求官 納畜 動土", @"忌":@"登高 行船 安床 入宅 博彩"};
        
    }else  if([d isEqualToString:@"成"]){ yjdict = @{@"宜":@"祈福 入学 开市 求医 成服", @"忌":@"词讼 安門 移徙"};
